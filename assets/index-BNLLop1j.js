(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();let f=null,p=[];async function B(a){try{const e="/brand-operating-site/".endsWith("/")?"/brand-operating-site/":"/brand-operating-site//",t=await fetch(`${e}brands/${a}/brand.json`);if(!t.ok)throw new Error(`Brand "${a}" not found`);return f=await t.json(),F(f.theme),f}catch(e){return console.error("Failed to load brand:",e),null}}function c(){return f}async function P(){if(p.length)return p;try{const a="/brand-operating-site/".endsWith("/")?"/brand-operating-site/":"/brand-operating-site//",e=await fetch(`${a}brands/brands-index.json`);e.ok?p=await e.json():p=[{id:"sample-brand",name:"Amore Creatives",nameKo:"아모레 크리에이티브"}]}catch{p=[{id:"sample-brand",name:"Amore Creatives",nameKo:"아모레 크리에이티브"}]}return p}function F(a){if(!a)return;const e=document.documentElement;e.style.setProperty("--primary",a.primaryColor),e.style.setProperty("--secondary",a.secondaryColor),e.style.setProperty("--accent",a.accentColor||a.secondaryColor),e.style.setProperty("--bg",a.backgroundColor),e.style.setProperty("--surface",a.surfaceColor),e.style.setProperty("--text",a.textColor),e.style.setProperty("--text-secondary",a.textSecondary),e.style.setProperty("--border",a.borderColor),a.fontHeading&&e.style.setProperty("--font-heading",a.fontHeading),a.fontBody&&e.style.setProperty("--font-body",a.fontBody)}const h={};let E=null;function v(a,e){h[a]=e}function b(){return window.location.hash.slice(1)||"/"}function T(a){async function e(){const t=b();if(t===E)return;E=t;const n=h[t]||h["/"];if(n){const s=document.getElementById("content");s&&(s.style.opacity="0",await new Promise(i=>setTimeout(i,120)),await n(s),s.style.opacity="1")}a&&a(t)}window.addEventListener("hashchange",e),e()}const j=[{section:"Foundation",items:[{path:"/",icon:"◎",label:"Brand Strategy"},{path:"/assets",icon:"◆",label:"Asset Library"}]},{section:"Interactive Guide",items:[{path:"/colors",icon:"●",label:"Color System"},{path:"/typography",icon:"Aa",label:"Typography"},{path:"/voice-tone",icon:"✦",label:"Voice & Tone"}]},{section:"Governance",items:[{path:"/versions",icon:"⟳",label:"Version History"}]}];function L(a){const e=c(),t=b();let n="";j.forEach(i=>{n+=`<div class="sidebar-section-label">${i.section}</div>`,i.items.forEach(o=>{const d=o.path===t?" active":"";n+=`
  ...
  `,V(e),U(e)}function V(a){const e=document.getElementById("logo-size-slider"),t=document.getElementById("logo-preview"),n=document.getElementById("logo-size-value"),s=document.getElementById("logo-margin-indicator"),i=document.getElementById("logo-preview-area");if(!e||!t)return;const o=a.checkerRules?.logoMinMarginPx||20;e.addEventListener("input",()=>{const d=e.value;t.style.fontSize=d*.4+"px",n.textContent=d+"px";const r=t.getBoundingClientRect(),l=i.getBoundingClientRect();s.style.width=r.width+o*2+"px",s.style.height=r.height+o*2+"px",s.style.left=r.left-l.left-o+"px",s.style.top=r.top-l.top-o+"px"}),document.querySelectorAll(".bg-preview-btn").forEach(d=>{d.addEventListener("click",()=>{i.style.background=d.dataset.bg;const r=d.dataset.bg==="#1B2838";t.style.color=r?"#FFFFFF":"var(--primary)"})})}function U(a){document.querySelectorAll(".asset-category-card").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.catId,n=a.assets?.categories?.find(i=>i.id===t);if(!n)return;if(n.externalLink){w(`${n.name}: 유료 폰트 구매 페이지로 이동합니다.`);return}const s=document.getElementById("asset-detail");s.style.display="block",s.innerHTML=`
      ...
      `,s.scrollIntoView({behavior:"smooth"})})})}function W(a){const e=c();if(!e)return;const{colors:t}=e;function n(d,r){return r.map(l=>`
  ...
  `,a.querySelectorAll(".copy-btn[data-copy]").forEach(d=>{d.addEventListener("click",r=>{r.stopPropagation(),x(d.dataset.copy)})}),a.querySelectorAll(".color-swatch").forEach(d=>{d.addEventListener("click",()=>{x(d.dataset.hex)})}),K(),_()}function K(){const a=document.getElementById("a11y-fg"),e=document.getElementById("a11y-bg"),t=document.getElementById("a11y-preview"),n=document.getElementById("a11y-result");if(!a||!e)return;function s(){t.style.color=a.value,t.style.background=e.value;const i=C(a.value,e.value),o=I(i);n.innerHTML=`
  ...
  `}async function Z(){await B("sample-brand"),L(document.getElementById("sidebar")),$(document.getElementById("header")),v("/",e=>G(e)),v("/assets",e=>D(e)),v("/colors",e=>W(e)),v("/typography",e=>X(e)),v("/voice-tone",e=>J(e)),v("/versions",e=>Q(e)),T(e=>{A(),$(document.getElementById("header"))});const a=document.getElementById("content");a&&(a.style.transition="opacity 0.12s ease")}Z();
