(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();let v=null,g=[];async function p(a="sample-brand"){if(v&&v.brandId===a)return v;const t=[`${"/brand-operating-site/".replace(/\/$/,"")}/brands/${a}/brand.json`,`./brands/${a}/brand.json`,`brands/${a}/brand.json`,`/brands/${a}/brand.json`];for(const n of t)try{const s=await fetch(n);if(s.ok)return v=await s.json(),S(v.theme),v}catch{}return console.error(`Failed to load brand "${a}" from all paths`),null}function c(){return v}async function P(){if(g.length)return g;const e=[`${"/brand-operating-site/".replace(/\/$/,"")}/brands/brands-index.json`,"./brands/brands-index.json","brands/brands-index.json","/brands/brands-index.json"];for(const t of e)try{const n=await fetch(t);if(n.ok)return g=await n.json(),g}catch{}return g=[{id:"sample-brand",name:"Amore Creatives",nameKo:"아모레 크리에이티브"}],g}function S(a){if(!a)return;const e=document.documentElement;e.style.setProperty("--primary",a.primaryColor||"#000000"),e.style.setProperty("--secondary",a.secondaryColor||"#000000"),e.style.setProperty("--accent",a.accentColor||"#000000"),e.style.setProperty("--bg",a.backgroundColor||"#FFFFFF"),e.style.setProperty("--surface",a.surfaceColor||"#FFFFFF"),e.style.setProperty("--text",a.textColor||"#000000"),e.style.setProperty("--text-secondary",a.textSecondary||"#666666"),e.style.setProperty("--border",a.borderColor||"#E8E8E8"),a.fontHeading&&e.style.setProperty("--font-heading",a.fontHeading),a.fontBody&&e.style.setProperty("--font-body",a.fontBody)}const $={};let z=null;function y(a,e){$[a]=e}function h(){return window.location.hash.slice(1)||"/"}function T(a){async function e(){const t=h();if(t===z)return;z=t;const n=$[t]||$["/"];if(n){const s=document.getElementById("content");s&&(s.style.opacity="0",await new Promise(i=>setTimeout(i,120)),await n(s),s.style.opacity="1")}a&&a(t)}window.addEventListener("hashchange",e),e()}const j=[{section:"Foundation",items:[{path:"/",icon:"◎",label:"Brand Strategy"},{path:"/assets",icon:"◆",label:"Asset Library"}]},{section:"Interactive Guide",items:[{path:"/colors",icon:"●",label:"Color System"},{path:"/typography",icon:"Aa",label:"Typography"},{path:"/voice-tone",icon:"✦",label:"Voice & Tone"}]},{section:"Governance",items:[{path:"/versions",icon:"⟳",label:"Version History"}]}];function L(a){const e=c(),t=h();let n="";j.forEach(i=>{n+=`<div class="sidebar-section-label">${i.section}</div>`,i.items.forEach(o=>{const d=o.path===t?" active":"";n+=`
  ...
  `}function A(){const a=h();document.querySelectorAll(".sidebar-link").forEach(e=>{e.classList.toggle("active",e.dataset.path===a)})}const H={"/":"Brand Strategy","/assets":"Asset Library","/colors":"Color System","/typography":"Typography","/voice-tone":"Voice & Tone","/versions":"Version History"};function w(a){const e=c(),t=h(),n=H[t]||"Brand Operating Site";a.innerHTML=`
  ...
  `,a.length===0?s.innerHTML=`<div style="padding: 16px; text-align: center; color: var(--text-muted); font-size: 13px;">
    ...
    `).join(""),a.style.position="relative",a.appendChild(n),n.querySelectorAll(".brand-option").forEach(s=>{s.addEventListener("click",async()=>{await p(s.dataset.brandId),L(document.getElementById("sidebar")),w(document.getElementById("header")),window.dispatchEvent(new HashChangeEvent("hashchange")),n.remove()})}),setTimeout(()=>{document.addEventListener("click",function s(i){a.contains(i.target)||(n.remove(),document.removeEventListener("click",s))})},10)})}async function G(a){let e=c();if(e||(e=await p("sample-brand")),!e){a.innerHTML='<div style="padding:40px; text-align:center; color:#999;">브랜드 데이터를 불러오는 중입니다... 잠시 후 새로고침 해주세요.</div>';return}const t=e.strategy;let n="";t.personality?.spectrum?.forEach(i=>{n+=`
  ...
  `}function E(a,e=2500){const t=document.getElementById("toast-container");if(!t)return;const n=document.createElement("div");n.className="toast",n.textContent=a,t.appendChild(n),setTimeout(()=>{n.classList.add("fade-out"),setTimeout(()=>n.remove(),300)},e)}function b(a){navigator.clipboard.writeText(a).then(()=>{E(`복사됨: ${a}`)}).catch(()=>{E("복사 실패")})}function B(a){const e=parseInt(a.slice(1,3),16),t=parseInt(a.slice(3,5),16),n=parseInt(a.slice(5,7),16);return{r:e,g:t,b:n}}function C(a,e){function t(l,m,f){const[x,k,I]=[l,m,f].map(u=>(u=u/255,u<=.03928?u/12.92:Math.pow((u+.055)/1.055,2.4)));return .2126*x+.7152*k+.0722*I}const n=B(a),s=B(e),i=t(n.r,n.g,n.b),o=t(s.r,s.g,s.b),d=Math.max(i,o),r=Math.min(i,o);return((d+.05)/(r+.05)).toFixed(2)}function F(a){return a>=7?{level:"AAA",pass:!0}:a>=4.5?{level:"AA",pass:!0}:a>=3?{level:"AA Large",pass:!0}:{level:"Fail",pass:!1}}async function D(a){let e=c();if(e||(e=await p("sample-brand")),!e){a.innerHTML='<div style="padding:40px; text-align:center; color:#999;">로딩 중...</div>';return}const t=e.assets?.categories||[];let n=t.reduce((i,o)=>i+o.count,0),s=t.map(i=>`
  ...
  `,V(e),U(e)}function V(a){const e=document.getElementById("logo-size-slider"),t=document.getElementById("logo-preview"),n=document.getElementById("logo-size-value"),s=document.getElementById("logo-margin-indicator"),i=document.getElementById("logo-preview-area");if(!e||!t)return;const o=a.checkerRules?.logoMinMarginPx||20;e.addEventListener("input",()=>{const d=e.value;t.style.fontSize=d*.4+"px",n.textContent=d+"px";const r=t.getBoundingClientRect(),l=i.getBoundingClientRect();s.style.width=r.width+o*2+"px",s.style.height=r.height+o*2+"px",s.style.left=r.left-l.left-o+"px",s.style.top=r.top-l.top-o+"px"}),document.querySelectorAll(".bg-preview-btn").forEach(d=>{d.addEventListener("click",()=>{i.style.background=d.dataset.bg;const r=d.dataset.bg==="#1B2838";t.style.color=r?"#FFFFFF":"var(--primary)"})})}function U(a){document.querySelectorAll(".asset-category-card").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.catId,n=a.assets?.categories?.find(i=>i.id===t);if(!n)return;if(n.externalLink){E(`${n.name}: 유료 폰트 구매 페이지로 이동합니다.`);return}const s=document.getElementById("asset-detail");s.style.display="block",s.innerHTML=`
      ...
      `,s.scrollIntoView({behavior:"smooth"})})})}async function W(a){let e=c();if(e||(e=await p("sample-brand")),!e)return;const{colors:t}=e;function n(d,r){return r.map(l=>`
    ...
    `).join("")}const s=[...t.primary||[],...t.secondary||[]];let i="";for(let d=0;d<s.length;d++)for(let r=d+1;r<s.length;r++){const l=s[d],m=s[r],f=C(l.hex,m.hex),x=F(f);i+=`
  ...
  `,a.querySelectorAll(".copy-btn[data-copy]").forEach(d=>{d.addEventListener("click",r=>{r.stopPropagation(),b(d.dataset.copy)})}),a.querySelectorAll(".color-swatch").forEach(d=>{d.addEventListener("click",()=>{b(d.dataset.hex)})}),K(),_()}function K(){const a=document.getElementById("a11y-fg"),e=document.getElementById("a11y-bg"),t=document.getElementById("a11y-preview"),n=document.getElementById("a11y-result");if(!a||!e)return;function s(){t.style.color=a.value,t.style.background=e.value;const i=C(a.value,e.value),o=F(i);n.innerHTML=`
    ...
    `}a.addEventListener("change",s),e.addEventListener("change",s),s()}function _(){const a=document.getElementById("grad-color1"),e=document.getElementById("grad-color2"),t=document.getElementById("grad-angle"),n=document.getElementById("grad-preview"),s=document.getElementById("grad-css"),i=document.getElementById("grad-copy-btn");if(!a||!e)return;function o(){const d=`linear-gradient(${t.value}deg, ${a.value} 0%, ${e.value} 100%)`;n.style.background=d,s.textContent=d}a.addEventListener("change",o),e.addEventListener("change",o),t.addEventListener("change",o),o(),i?.addEventListener("click",()=>{b(s.textContent)})}async function X(a){let e=c();if(e||(e=await p("sample-brand")),!e)return;const{typography:t}=e;function n(i,o){return`
      <div class="card">
  ...
  `}async function Q(a){let e=c();if(e||(e=await p("sample-brand")),!e)return;const t=e.versions||[],n=t[0];let s=t.map(i=>`
  ...
  `}async function Z(){await p("sample-brand"),L(document.getElementById("sidebar")),w(document.getElementById("header")),y("/",e=>G(e)),y("/assets",e=>D(e)),y("/colors",e=>W(e)),y("/typography",e=>X(e)),y("/voice-tone",e=>J(e)),y("/versions",e=>Q(e)),T(e=>{A(),w(document.getElementById("header"))});const a=document.getElementById("content");a&&(a.style.transition="opacity 0.12s ease")}Z();
